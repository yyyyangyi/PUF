# Modified from https://github.com/lyuwenyu/RT-DETR/blob/b6bf0200b249a6e35b44e0308b6058f55b99696b/rtdetrv2_pytorch/src/zoo/rtdetr/rtdetrv2_decoder.py
...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        attn_output, attn_queries, attn_keys = self.multi_head_attention_forward(
        ...
        if self.batch_first and is_batched:
            return attn_output.transpose(1, 0), attn_queries, attn_keys
        else:
            return attn_output, attn_queries, attn_keys
    ...
    def multi_head_attention_forward(
        ...
        is_batched = F._mha_shape_check(query, key, value, None, attn_mask, num_heads)
        if not is_batched:
        ...
        q_scaled = q_scaled.view(bsz, num_heads, tgt_len, head_dim)
        k = k.view(bsz, num_heads, src_len, head_dim)

        if not is_batched:
            # squeeze the output if input was unbatched
            attn_output = attn_output.squeeze(1)
        return attn_output, q_scaled, k
...
class TransformerDecoderLayer(nn.Module):
    def __init__(self,
        ...
        # self attention
        self.self_attn = MultiheadAttention(d_model, n_head, dropout=dropout, batch_first=True)
    ...
    def forward(self,
        ...
        target2, self_attn_queries, self_attn_keys = self.self_attn(q, k, value=target, attn_mask=attn_mask)
        ...
        return target, self_attn_queries, self_attn_keys
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self,
        ...
        for i, layer in enumerate(self.layers):
            ...
            output, attn_queries, attn_keys = layer(output, ref_points_input, memory, memory_spatial_shapes, attn_mask, memory_mask, query_pos_embed)
            inter_ref_bbox = F.sigmoid(bbox_head[i](output) + inverse_sigmoid(ref_points_detach))

            dec_attn_queries.append(attn_queries)
            dec_attn_keys.append(attn_keys)
        ...
        return torch.stack(dec_out_bboxes), torch.stack(dec_out_logits), output, dec_attn_queries, dec_attn_keys
...
class RTDETRTransformerv2(nn.Module):
    ...
    def forward(self, feats, targets=None):
        ...
        # decoder
        out_bboxes, out_logits, last_hidden_state, attn_queries, attn_keys = self.decoder(
            init_ref_contents,
        ...
        if self.training and dn_meta is not None:
            ...
            _, last_hidden_state = torch.split(last_hidden_state, dn_meta['dn_num_split'], dim=1)
            attn_queries = [torch.split(q, dn_meta['dn_num_split'], dim=2)[1] for q in attn_queries]
            attn_keys = [torch.split(k, dn_meta['dn_num_split'], dim=2)[1] for k in attn_keys]

        attn_queries = torch.stack(attn_queries, dim=0)
        attn_keys = torch.stack(attn_keys, dim=0)

        out = {'pred_logits': out_logits[-1], 'pred_boxes': out_bboxes[-1], 'last_hidden_state': last_hidden_state, "decoder_attention_queries": attn_queries, "decoder_attention_keys": attn_keys}
